...
`address[10:0]` به ورودی PC `next_address` داده می‌شود. FSM برای jump گرفته‌شده هر دو `pc_enable` و `pc_load` را assert می‌کند؛ بنابراین PC این target را در execute load می‌کند.
