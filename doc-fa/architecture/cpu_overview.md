...
entity با نام `cpu_top`، `control_unit` یا `status_register` وجود ندارد؛ این نام‌های تاریخی RTL فعلی نیستند. interface memory خارجی، Pipeline و jump redirection پیاده‌سازی‌شده وجود ندارد.
